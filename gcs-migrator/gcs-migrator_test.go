...
var (
	...
	srcBucket         = flag.String("src-bucket", "gcs-test-migration", "Source bucket to migrate objects from")
...
func TestGcsMigrator(t *testing.T) {
	...
	if stream, err = migrationpb.NewServiceClient(conn).Migrate(ctx); err != nil {
	...
	t.Logf("client.Migrate: %d migrations, %d batches", len(response.Migrations), response.Batches)
